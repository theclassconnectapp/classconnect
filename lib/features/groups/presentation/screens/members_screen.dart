...
Future<void> _memberActions(
  ...
  if (action == 'promote') {
    try {
      ...
      if (!context.mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Failed to promote member. Please try again.')),
      );
    }
    return;
  }
  if (action == 'mute') {
    try {
      ...
      if (!context.mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('$memberName has been muted.')),
      );
    } catch (e) {
      if (!context.mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Failed to mute member. Please try again.')),
      );
    }
    return;
  }
  if (action == 'unmute') {
    try {
      ...
      if (!context.mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Failed to unmute member. Please try again.')),
      );
    }
    return;
  }
